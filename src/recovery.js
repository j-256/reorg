...
export function acquireRecoveryLock(filename) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (!fs.lstatSync(filename).isFile()) throw new Error('Recovery lock is not a regular file');
      const lock = JSON.parse(fs.readFileSync(filename, 'utf8'));
      if (!Number.isInteger(lock.pid) || lock.pid <= 0 || processExists(lock.pid)) {
...
function restoreRun(scriptPath, stamp) {
  ...
  if (!fs.lstatSync(journalPath).isFile()) throw new Error('Recovery journal is not a regular file');
...
export function buildUndoScript(ops, stamp, opts = {}) {
  if (!/^[A-Za-z0-9_-]+$/.test(stamp)) throw new Error('Invalid apply run identifier');
  const helpers = [directoryProblem, entryIdentity, saveJournal, processExists, acquireRecoveryLock, recoveryPath, restoreRun];
  return [
    '#!/bin/bash',
