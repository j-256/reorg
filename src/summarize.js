...
import { loadPlan, savePlan, ensureStateDir } from './state.js';
 ...
 */
export async function summarize({ root, paths, model = DEFAULT_MODEL, force = false, onProgress }) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    return {
      error:
        'ANTHROPIC_API_KEY is not set. Either export it, or run `reorg summarize --emit-prompts` ' +
        'and let your coding agent fill in .reorg/summaries.json (no key needed).',
      summaries: {},
      skipped: [],
    };
  }
  ...
  if (Object.keys(summaries).length) {
    savePlan(root, { ...plan, summaries: { ...plan.summaries, ...summaries } });
  }
 ...
 */
export async function emitPrompts({ root, paths, force = false }) {
  const plan = loadPlan(root);
  const { entries, skipped } = await collect({ root, paths, existing: plan.summaries, force });
...
export function ingestSummaries(root) {
  const p = join(root, '.reorg', OUT_FILE);
  let raw;
  try {
  ...
  const plan = loadPlan(root);
  const merged = { ...plan.summaries };
  let added = 0;
  for (const [k, v] of Object.entries(raw)) {
    if (typeof v === 'string' && v.trim()) {
      merged[k] = v.trim().slice(0, 200);
      added++;
    }
  }
  savePlan(root, { ...plan, summaries: merged });
