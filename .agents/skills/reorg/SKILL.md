...
Nested moves and directory creation can require staging even without a simple name swap. Preserve the resolver's operation order and treat staging slot names as opaque recovery paths. Preflight follows descendants through planned parent moves so collisions carried by a directory are checked before application.
